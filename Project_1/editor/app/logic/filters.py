...
class Filters:
    ...
    def negative(self, image: Image.Image) -> Image.Image:
        img = np.array(image)
        rows, cols = img.shape[:2]
        neg = np.zeros((rows, cols, 3), dtype='uint8')
        for i in range(rows):
            for j in range(cols):
    ...
    # Wybieramy wielkość kernela i sigma
    ...
    # Ustalamy własny kernel jako macierz 2D 
    ...
    # Poniższa funkcja dla erozji, dylatacji, otwarcia i zamknięcia pozwala na wybór kształtu elementu strukturyzującego (SE) oraz jego rozmiaru. 
    def _get_structuring_element(self, size: int, shape: str) -> np.ndarray:
        ...
        elif shape == 'ellipse': #koło wpisane w kwadrat
            for i in range(size):
                for j in range(size):
    ...
    def hit_or_miss(self, image: Image.Image, preset: str = "isolated", threshold: int = 128) -> Image.Image:
        ...
        presets = {
            ...
            "square_2x2": (
                ...
                np.array([[0,0,1],[0,0,1],[1,1,1]], dtype=np.uint8),
            ),
            "cross": (
            ...
            "diagonal_br": (
                ...
                np.array([[1,1,1],[1,0,0],[1,0,0]], dtype=np.uint8),
            ),
        }

        se1, se2 = presets.get(preset, presets["isolated"])
        ...
        eroded1 = self._erode(binary * 255, se1)
        eroded2 = self._erode(inv    * 255, se2)
        result  = np.minimum(eroded1, eroded2)

        return Image.fromarray(result.astype('uint8')).convert('RGB')

    def skeletonize(self, image: Image.Image, threshold: int = 128) -> Image.Image:
        gray    = self.convert_to_gray_avg(image)
        img     = np.array(gray.convert('L'))
        current = (img > threshold).astype(np.uint8) * 255 
